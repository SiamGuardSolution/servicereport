...
async function tryAcrossBases({ method, bases, keys, body, qs }) {
  ...
  // GET ใช้ route อย่างเดียว, POST ลองทั้ง path และ route
  ...
  for (const base of bases) {
    for (const key of keys) {
      for (const paramKey of paramVariants) {
        try {
          const queryStr = new URLSearchParams(isGET && qs ? qs : {}).toString();
          const url = isGET
            ? `${base}?${paramKey}=${encodeURIComponent(key)}${queryStr ? `&${queryStr}` : ''}`
          ...
          if (!unknown) {
            ...
            // แนบเมทาเล็กน้อยไว้ดีบัก
            return Object.assign(out, { __used: { base, key, paramKey } });
          }
