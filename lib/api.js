...
function authHeader() {
  try {
    const { token } = getAuth() || {};
    return token ? { 'X-Auth-Token': String(token) } : {};
  } catch {
    return {};
  }
}
...
async function tryAcrossBasesAndRoutes({ method, bases, routes, body, qs }) {
  const errors = [];
  const ts = Date.now(); // กันแคชทุกครั้ง
  ...
  for (const base of bases) {
    for (const route of routes) {
      try {
        ...
        const init =
          method === 'GET'
            ? { method: 'GET', redirect: 'follow', cache: 'no-store', headers: { ...authHeader() } }
            : {
                method: 'POST',
        ...
        // ตีความ “ไม่รู้จัก route/404” หลายแบบ
        const unknown =
          !res.ok ||
          (json && json.ok === false && /unknown route|not\s*found/i.test(json.error || '')) ||
          (!json && /unknown route|not\s*found/i.test(String(raw || '')));

        if (!unknown) {
          // ถ้า JSON ไม่มี ok ให้ถือว่าโอเค (GAS หลายตัวไม่ใส่ ok)
          return json ?? { ok: true, raw };
        }
        errors.push({ base, route, status: res.status, error: json?.error || raw?.slice(0, 160) });
      } catch (e) {
...
export const teamsList   = () =>
  tryAcrossBasesAndRoutes({ method: 'GET',  bases: resolveExecBases(), routes: ['teams.list'], qs: {} });

export const staffUpsert = (payload) =>
  tryAcrossBasesAndRoutes({ method: 'POST', bases: resolveExecBases(), routes: ['staff.upsert'], body: payload });
...
export async function uploadImages(filesWithZone = []) {
  ...
  for (const it of filesWithZone) {
    const dataUrl = await fileToDataURL(it.file); // ส่ง dataURL เต็ม ๆ ให้ GAS
    const data = await tryAcrossBasesAndRoutes({
      method: 'POST',
      bases: resolveExecBases(),
