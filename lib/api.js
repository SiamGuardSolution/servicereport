...
async function tryAcrossBases({ method, bases, keys, body, qs }) {
  ...
  // GET ใช้ ?route= เท่านั้น / POST ลองทั้ง ?path= และ ?route=
  const paramVariants = isGET ? ['route'] : ['path', 'route'];

  // สร้าง query string เฉพาะ key ที่มีค่าเท่านั้น
  const makeQuery = (obj = {}) =>
    Object.entries(obj)
    ...
    for (const key of keys) {
      for (const paramKey of paramVariants) {
        try {
          ...
          const url = isGET
            ? `${baseNoSlash}?${paramKey}=${encodeURIComponent(key)}${qUser ? `&${qUser}` : ''}`
            : `${baseNoSlash}?${paramKey}=${encodeURIComponent(key)}`;

          // IMPORTANT: GET แบบ “ไม่มี header ใดๆ”
          /** @type {RequestInit} */
          const init = isGET
            ? { method: 'GET' } // ไม่มี headers / body / cache / credentials ใดๆ ทั้งสิ้น
            : {
                method: 'POST',
                headers: { 'Content-Type': 'text/plain; charset=utf-8' }, // หลบ preflight
                body: JSON.stringify(body || {})
              };

          // ป้องกันกรณีถูกสอดไส้ headers ที่อื่นมาโดยไม่ตั้งใจ
          if (isGET && init.headers) delete init.headers;

          const res = await withTimeout(fetch(url, init));
          const { json, raw } = await parseMaybeJSON(res);

          // ถ้าไม่ใช่ “unknown route/path” ถือว่าสำเร็จ
          const isUnknown =
            !res.ok ||
          ...
          if (!isUnknown) {
            ...
            return Object.assign(out, { __used: { base: baseNoSlash, key, paramKey, method: isGET ? 'GET' : 'POST' } });
          }

          errors.push({ base: baseNoSlash, key, paramKey, status: res.status, error: json?.error || String(raw || '').slice(0, 160) });
        } catch (e) {
          errors.push({ base: baseNoSlash, key, paramKey, status: 'ERR', error: String(e) });
        }
      }
    }
  }
